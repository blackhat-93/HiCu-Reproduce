...
def save_test_results(metrics_hist_te, model_dir):
    with open(model_dir + "/test_results.csv", mode="w", newline='') as file:
        ...
        paper_results = {key: metrics_hist_te[key][0] for key in paper_metrics if key in metrics_hist_te}
...
def save_everything(args, metrics_hist_all, model, model_dir, params, criterion, test_only=False):
    ...
    if len(metrics_hist_all[1]) != 0:
        # Save test results to a separate file
        save_test_results(metrics_hist_all[1], model_dir)
